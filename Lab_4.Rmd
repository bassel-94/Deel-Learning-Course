...
perceptron.train = function(x, y, eta, niter, normal_vector, intercept) {
        ...
        for (j in 1:niter) {
                ...
                for (i in 1:length(y)) {
                        ...
                        # Change weight - but do anything if the predicted value is correct
...
We notice that we get an impressive performance of 94% when applying a perceptron to a dataset that is not linearly separable. Clearly this is due to the fact that we added some polynomial combination of the variables. We know that the classes are not linearly separable. However, we projected the data in a higher dimension and separated them linearly.
...
a. Generate and visualize data from distributional setting 1
...
```{r}
#-- Making a prediction in neural networks
...
The error, we expected from previous parts in the labs, is still around 30%. I did not perform a test train split for this part. The error computed is the training error.
...
```{r}
#-- build lda model and make predictions
library(MASS)
...
**TODO** get the LDA normal vector and compare it to the normal vector in neural network. Calculate the angle using dot product.
